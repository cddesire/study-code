...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        # 在这里先将不同时刻LSTM结构的输出收集起来，再通过一个全连接层得到最终的输出
        outputs = []
        ...
        # 把输出队列展开成[batch_size, hidden_size * num_steps]的形状，然后再
        # reshape成[batch_size * num_steps, hidden_size]的形状
...
class SmallConfig(object):
    ...
    num_layers = 2  # lstm层数
    num_steps = 20  # 序列的长度，为了避免梯度消散的问题，会规定一个最大的序列长度
...
class LargeConfig(object):
    """Large config."""
    ## 2个LSTM层，展开大小是35，每层包含了1500个隐藏节点
    init_scale = 0.04
    learning_rate = 1.0
    max_grad_norm = 10
    num_layers = 2
